                ...
                if (account != null) {
                    if (SecurityUtils.verifyPassword(accountPassword, account.getAccountPassword())) {
                        ...
                        if (account.getAccountStatus() == AccountStatus.LOCKED) {
                            ...
                            if (lockedTime == null && account.getDurationLocked() == Integer.MAX_VALUE)
                                throw new AccountLockedException(account.getAccountId(), Integer.MAX_VALUE);
    ...
    public Admin adminLogin (Map<Integer, Admin> admins, Owner owner) {
        ...
        try {
            for (int i = 0; i < 3; i++) {
                ...
                if (admin != null && SecurityUtils.verifyPassword(adminPassword, admin.getAdminPassword()))
                    return admin;
